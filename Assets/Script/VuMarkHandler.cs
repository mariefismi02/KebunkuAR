...
public class VuMarkHandler : MonoBehaviour {
    ...
    public class AugmentationObject {
        ...
        public string vumarkObjName;
        public string vumarkDesc;
        public Sprite vumarkIcon;
    ...
    public void OnVuMarkLost(VuMarkTarget vumarkTarget) {
        ...
        if (vumarkTarget == this.currentVuMark)
            this.nearestVuMarkScreenPanel.Hide();
    }
    ...
    string GetNumericVuMarkDescription(VuMarkTarget vumarkTarget) {
        ...
        if (int.TryParse(GetVuMarkId(vumarkTarget), NumberStyles.Integer, CultureInfo.InvariantCulture, out vumarkIdNumeric)) {
            ...
            return augmentationObjects[vumarkIdNumeric].vumarkObjName;
        }

        return string.Empty; // if VuMark DataType is byte or string
    }

    string GetVuMarkDescription(VuMarkTarget vumarkTarget)
    {
        int vumarkIdNumeric;
        ...
        {
            // Change the description based on the VuMark ID
            return augmentationObjects[vumarkIdNumeric].vumarkDesc;
        }

        return string.Empty; // if VuMark DataType is byte or string
    }

    Sprite GetVuMarkIcon(VuMarkTarget vumarkTarget)
    {
        int vumarkIdNumeric;
        ...
        {
            // Change the description based on the VuMark ID
            return augmentationObjects[vumarkIdNumeric].vumarkIcon;
        }

        return augmentationObjects[0].vumarkIcon; // if VuMark DataType is byte or string
    }
    ...
    void SetVuMarkAugmentation(VuMarkBehaviour vumarkBehaviour) {
        ...
        if (sourceAugmentation) {
            ...
            augmentation.transform.localPosition = objectsPosition;
            augmentation.transform.localScale = Vector3.one * objectsScale;
    ...
    IEnumerator ShowPanelAfter(float seconds, string vuMarkId, string vuMarkDataType, string vuMarkName, string vuMarkDesc, Sprite vuMarkImage, Sprite vuMarkIcon) {
        yield return new WaitForSeconds(seconds);

        if (this.nearestVuMarkScreenPanel) {
            nearestVuMarkScreenPanel.Show(vuMarkId, vuMarkDataType, vuMarkName, vuMarkDesc, vuMarkImage, vuMarkIcon);
        }
    }
